...
from slidegen import crud
from slidegen.api.deps import AsyncDB, CurrentUser, get_current_user
from slidegen.core import security, settings
from slidegen.schemas.auth import LoginPayload, LoginResponse, RefreshResponse
from slidegen.schemas.user import UserCreate, UserPublic, UserRegister

router = APIRouter()


@router.post("/login", response_model=LoginResponse)
async def login(session: AsyncDB, payload: LoginPayload) -> Any:
    user = await crud.authenticate(session=session, username=payload.username, password=payload.password)
    if not user:
...
async def register_user(session: AsyncDB, user_in: UserRegister) -> Any:
    """
