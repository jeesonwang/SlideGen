...
class BaseSchemaValidator:
    ...
    def __init__(self, unpacked_dir: str | Path, original_file: str | Path, verbose: bool = False) -> None:
    ...
    def validate_unique_ids(self) -> bool:
        ...
        errors: list[str] = []
        global_ids: dict[str, tuple[Path, int, str]] = {}  # Track globally unique IDs across all files

        for xml_file in self.xml_files:
            try:
                root = lxml.etree.parse(str(xml_file)).getroot()
                file_ids: dict[tuple[str, str], dict[str, int]] = {}  # Track IDs that must be unique within this file
    ...
    def _clean_ignorable_namespaces(self, xml_doc: lxml.etree.ElementTree) -> lxml.etree.ElementTree:
    ...
    def _remove_ignorable_elements(self, root: lxml.etree.ElementTree) -> None:
    ...
    def _preprocess_for_mc_ignorable(self, xml_doc: lxml.etree.ElementTree) -> lxml.etree.ElementTree:
    ...
    def _validate_single_file_xsd(self, xml_file: str | Path, base_path: str | Path) -> tuple[bool | None, set[str]]:
        """Validate a single XML file against XSD schema. Returns (is_valid, errors_set)."""
        xml_file = Path(xml_file)
        schema_path = self._get_schema_path(xml_file)
        if not schema_path:
            return None, set()  # Skip file
    ...
    def _get_original_file_errors(self, xml_file: str | Path) -> set[str]:
    ...
    def _remove_template_tags_from_text_nodes(
        self, xml_doc: lxml.etree.ElementTree
    ) -> tuple[lxml.etree.ElementTree, list[str]]:
        ...
        def process_text_content(text: str, content_type: str) -> str:
            if not text:
                return text
