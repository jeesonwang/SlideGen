...
ChapterContentPage.generate_slide()
  ...
  -> 按 ComponentContentType 处理 title/content/picture/number/icon/decoration
```
...
- 对内容页中的 shape 做内容组识别和 `ComponentContentType` 标注，覆盖 `TITLE`、`CONTENT`、`NUMBER`、`PICTURE`、`ICON`、`DECORATION`。
...
- 当 style 中存在 `ComponentContentType.TITLE` 时，`len(title_shape.location)` 必须等于 point 数。
- 当 style 中存在 `ComponentContentType.CONTENT` 时，`len(content_shape.location)` 必须等于 point 数。
